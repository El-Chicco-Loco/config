...
wallpaper=$HOME/Pictures/wallpaper/wallpaper.GIF
...
awww="awww img"
effect="--transition-bezier .43,1.19,1,.4 --transition-fps 10 --transition-type grow --transition-pos 0.925,0.977 --transition-duration 2 --transition-step 5 --transition-fps 10 --transition-type simple"
...
if [ ! -f "$HOME/.config/hypr/.initial_startup_done" ]; then
	...
	if [ -f "$wallpaper" ]; then
		...
		awww query || awww-daemon && awww img $wallpaper $effect
